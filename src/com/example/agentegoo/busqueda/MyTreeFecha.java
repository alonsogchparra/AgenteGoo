...
public class MyTreeFecha {
	...
	public MyTreeFecha(Evento elem) {
		if (root == null)
	 ...
	 */
	private Node deleteNode(Node subtree, Calendar elem) {
	 ...
	 */ 	
	public Node findNode(Calendar item) {
		if (item == null)
			return null;
		Node current = root;
		while ((current.getElem().getFecha()).compareTo(item) != 0) {
			if ((current.getElem().getFecha()).compareTo(item) > 0) {
				current = current.leftChild;
			} else if ((current.getElem().getFecha()).compareTo(item) < 0) {
	...
	public static void main(String[] args) {
		...
		MyTreeFecha tree = new MyTreeFecha(new Evento("aaa",
		...
		//
		// System.out.println("\n"
		// + tree.findNode(new GregorianCalendar(2013, 2, 1, 10,
		// 49)).getElem().getNombre()
		// + "\nFecha: ");
		//
		// // Eliminar Evento
		// tree.delete(new GregorianCalendar(2013, 2, 1, 9, 45));
		// l = tree.depthFirstTraversal();
		// System.out.println("Busqueda en PROFUNDIDAD despues de Eliminar");
		// printTree(l);
